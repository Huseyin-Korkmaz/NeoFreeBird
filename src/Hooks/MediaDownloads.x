...
static NSArray *DMVideoEntities(UIView *attachmentView) {
    ...
    EnumerateSubviewsRecursively(attachmentView, ^(UIView *view) {
        ...
        for (TFSTwitterMediaInfo *info in [(_TtC21TweetMediaAttachments14MultiMediaView *)view inlineMediaInfos]) {
    ...
    if ([BHTSettings boolForKey:@"tweet_to_image"]) {
        if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
            ...
            if (tweetView == nil) {
                ...
                while (ancestor && ![ancestor isKindOfClass:UITableViewCell.class] && ![ancestor isKindOfClass:UICollectionViewCell.class]) {
