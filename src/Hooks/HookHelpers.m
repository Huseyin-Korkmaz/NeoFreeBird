...
void EnumerateSubviewsRecursively(UIView *view, void (^block)(UIView *currentView)) {
    ...
    // Depth cap; a static counter is fine since traversal only runs on the main thread.
...
// Module content reaches the section arrays wrapped in TFNDataViewItem (the real
// view model is its -item); standalone timeline items are the view model directly.
