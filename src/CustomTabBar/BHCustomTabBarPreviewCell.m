...
@interface BHCustomTabBarPreviewCell ()
@property (nonatomic, strong) UIView *shadowBox;
@property (nonatomic, strong) UIImageView *iconView;
@end

@implementation BHCustomTabBarPreviewCell
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.shadowBox.layer.shadowColor = BHCustomTabBarShadowColor().CGColor;
...
- (void)configureWithImageName:(NSString *)imageName {
    if (imageName.length) {
        self.iconView.image = [UIImage tfn_vectorImageNamed:imageName fitsSize:CGSizeMake(24, 24) fillColor:BHCustomTabBarIconColor()];
    } else {
...
+ (NSString *)reuseIdentifier {
    return @"BHCustomTabBarPreviewCell";
}
